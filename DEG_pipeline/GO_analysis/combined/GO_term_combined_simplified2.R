...
for (study in study_results[c(1, 5)]){
	...
	for (cell_type in names(chimp_down_regulated)){
		...
		chimp_combined_regulated_GO_cell_type[[cell_type]] <- simplify(chimp_combined_regulated_GO_cell_type[[cell_type]],  cutoff = 0.7,  by = "p.adjust",  select_fun = min,  measure = "Wang",  semData = NULL)
                human_combined_regulated_GO_cell_type[[cell_type]] <- simplify(human_combined_regulated_GO_cell_type[[cell_type]],  cutoff = 0.7,  by = "p.adjust",  select_fun = min,  measure = "Wang",  semData = NULL)
