...
pseudo_bulk_ready_path     <- "~/Yi_Lab/Lab_WorkDir/Dennis/Evolution/pbds/pseudo_bulk_ready_data_003/"
support_functions_path     <- "~/Yi_Lab/Lab_WorkDir/Dennis/pseudo_single_pipeline/support_functions/"
results_path               <- "~/Yi_Lab/Lab_WorkDir/Dennis/Evolution/pbres/pseudo_bulk_analysis_results_005BT/"
bootstrapped_gene_selection_matrix_list <- readRDS("~/Yi_Lab/Lab_WorkDir/Dennis/Evolution/rds_files/bootstrapped_gene_selection_matrix_list_Ma.rds")

current_path               <- this.path()
bootstrap_Step             <- as.numeric(strsplit(strsplit(current_path, "_")[[1]][8], ".R")[[1]])

print(bootstrap_Step)
source(paste(support_functions_path, "get_dds_resultsAvsB_boot_compact_age_sex_005.R", sep = ""))
